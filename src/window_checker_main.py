...
def main() -> int:
    ...
    focus_interval_minutes = rdp_cfg.get("focus_interval_minutes", 15)
    ...
    while True:
        ...
        if now - last_rdp_cycle >= focus_interval_seconds:
            ...
            try:
                reconnected, relaunched = cycle_or_recover_rdp_windows(title_search, log, beat=_beat)
                if reconnected > 0:
                    print(f"!! Reconnected {reconnected} RDP session(s) this cycle")
                if relaunched:
                    print("!! Relaunched host RDP stack")
                if reconnected == 0 and not relaunched:
                    print("OK RDP cycle complete — nothing to relaunch")
            except Exception:
